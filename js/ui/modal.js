...
export function fieldText(name, label, { value = "", type = "text", inputmode = "", hint = "", required = false, placeholder = "", disabled = false } = {}) {
  return `<div class="field">
    <label for="f_${name}">${esc(label)}${required ? " *" : ""}</label>
    <input type="${type}" ${inputmode ? `inputmode="${inputmode}"` : ""} id="f_${name}" name="${name}" value="${esc(value)}" placeholder="${esc(placeholder)}" ${disabled ? "disabled" : ""} />
